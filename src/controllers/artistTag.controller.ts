...
export class ArtistTagController {
  static create: RequestHandler<{ artistId: string; tagId: string }> = async (
    req,
    res,
    next
  ) => {
  ...
  static FindArtistTagByArtistIdAndTagId: RequestHandler<{
    artistId: string;
    tagId: string;
  }> = async (req, res, next) => {
  ...
  static FindArtistTagByArtistId: RequestHandler<{ artistId: string }> = async (
    req,
    res,
    next
  ) => {
    const artistTag = await db.query.artistTags.findFirst({
      where: and(eq(schema.artistTags.artistId, req.params.artistId)),
    });
  ...
  static FindArtistTagBytagId: RequestHandler<{ tagId: string }> = async (
    req,
    res,
    next
  ) => {
    const artistTag = await db.query.artistTags.findFirst({
      where: and(eq(schema.artistTags.tagId, req.params.tagId)),
    });
  ...
  static FindArtistTagById: RequestHandler<{ id: string }> = async (
    req,
    res,
    next
  ) => {
    const artistTag = await db.query.artistTags.findFirst({
      where: and(eq(schema.artistTags.id, req.params.id)),
    });
